...
$(function(){ //attention blocage CORS donc ajout d'extention "Allow-Control-Allow-Origin: *" à chrome
    $.get('http://127.0.0.1:3000/data_init', {},function(data_init){// init tab
        var j = 0;
        for (i=9; i>=0 ;i--){
            Temperature[i] = data_init[i].Temperature;
            Humidite[i] = data_init[i].Humidite;
            Luminosite[i] = data_init[i].Luminosite;
            Fertilite[i] = data_init[i].Fertilite;
            j++;
        }
